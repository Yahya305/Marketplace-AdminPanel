import React, { useState } from "react";
import { MdNotifications } from "react-icons/md";
import Dropdown from "react-bootstrap/Dropdown";
import { RxDashboard } from "react-icons/rx";
import { MdGroups } from "react-icons/md";
import { BiDollarCircle, BiLogOut } from "react-icons/bi";
import { BiMessageSquareDetail } from "react-icons/bi";
import { AiOutlineDollarCircle } from "react-icons/ai";
import { GrLogout } from "react-icons/gr";
import { CiViewList } from "react-icons/ci";
import { BiWalletAlt } from "react-icons/bi";
import { BiUserCircle } from "react-icons/bi";
import { useNavigate } from "react-router-dom";
// import MobileSideBar from "./MobileSideBar";
// import Feedback from "../Profile/Feedback"
import Offcanvas from "react-bootstrap/Offcanvas";
// import Notifications from "./Notifications";
import "./index.css";
import MobileSideBar from "./MobileSideBar";

export default function index({ children }) {
  const navigate = useNavigate();
  const [modalShow, setModalShow] = useState(false);
  const [showNotif, setShowNotif] = useState(false);

  const handleClose = () => setShowNotif(false);
  const handleShow = () => setShowNotif(true);
  return (
    <div id="large-container">
      <div className="mob-side-nav">
        <MobileSideBar />
      </div>
      <div className="sidebar">
        <div className="d-flex justify-content-between mx-3 align-items-center mt-3 p-2 text-white">
          <img
            role="button"
            onClick={() => navigate("/")}
            src="data:image/png;base64,"
          ></img>
          {/* onClick={() => navigate("/marketplace/notifications")} */}
          <div onClick={handleShow}>
            <MdNotifications className="notif-icon" />
          </div>
        </div>
        <hr style={{ color: "white" }}></hr>
        <div className="py-2 ps-3 mb-3">
          <ul className="nav nav-pills flex-column mt-4">
            <li
              role="button" className="nav-item d-flex align-items-center flex-row "
              onClick={() => navigate("/dashboard")}
            >
              <RxDashboard className="sidebar-icn" />
              <div className="px-2 sidebar-btn py-2 rounded">Dashboard</div>
            </li><li
              role="button" className="nav-item d-flex align-items-center flex-row "
              onClick={() => navigate("/iam")}
            >
              <RxDashboard className="sidebar-icn" />
              <div className="px-2 sidebar-btn py-2 rounded">IAM</div>
            </li>
            <li
              role="button" className="nav-item d-flex align-items-center flex-row "
              onClick={() => navigate("/group-policy")}
            >
              <MdGroups className="sidebar-icn" />
              <div className="px-2 sidebar-btn py-2 rounded">Group Policy</div>
            </li>
            <li
              role="button" className="nav-item d-flex align-items-center flex-row "
              onClick={() => navigate("/buyers")}
            >
              <BiWalletAlt className="sidebar-icn" />
              <div className="px-2 sidebar-btn py-2 rounded">Buyers</div>
            </li><li
              role="button" className="nav-item d-flex align-items-center flex-row "
              onClick={() => navigate("/sellers")}
            >
              <AiOutlineDollarCircle className="sidebar-icn" />
              <div className="px-2 sidebar-btn py-2 rounded">Sellers</div>
            </li><li
              role="button" className="nav-item d-flex align-items-center flex-row "
              onClick={() => navigate("/listings")}
            >
              <CiViewList className="sidebar-icn" />
              <div className="px-2 sidebar-btn py-2 rounded">Listings</div>
            </li><li
              role="button" className="nav-item d-flex align-items-center flex-row "
              onClick={() => navigate("/meetings")}
            >
              <BiWalletAlt className="sidebar-icn" />
              <div className="px-2 sidebar-btn py-2 rounded">Meetings</div>
            </li><li
              role="button" className="nav-item d-flex align-items-center flex-row "
              onClick={() => navigate("/deals")}
            >
              <BiMessageSquareDetail className="sidebar-icn" />
              <div className="px-2 sidebar-btn py-2 rounded">Deal</div>
            </li>
            <li
              role="button" className="nav-item d-flex align-items-center flex-row "
              onClick={() => navigate("/blogs/articles")}
            >
              <BiMessageSquareDetail className="sidebar-icn" />
              <div className="px-2 sidebar-btn py-2 rounded">Blogs</div>
            </li><li
              role="button" className="nav-item d-flex align-items-center flex-row "
              onClick={() => navigate("/logout")}
            >
              <BiLogOut className="sidebar-icn" />
              <div className="px-2 sidebar-btn py-2 rounded">Logout</div>
            </li>
          </ul>
        </div>
      </div>
      <div className="content-sec">{children}</div>
    </div>
  );
}
